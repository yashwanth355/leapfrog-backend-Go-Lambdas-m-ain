...
func storeAndEmailDocs(request Input, db *sql.DB) error {
	...
	if processingErr == nil {
		...
		//dnNumber, processingErr = debitnoteNumberById(request.Debitnoteid, db)
		processingErr = addContextData(&request, db)
	}
	if processingErr == nil {
		//request.DebitnoteNumber = dnNumber
...
func addContextData(request *Input, db *sql.DB) error {
	...
	/*query := `select debitnoteno from dbo.inv_gc_debitnote_master_newpg where debitnoteid=$1`
	rows, err := db.Query(query, request.Debitnoteid)
	if err == nil {
		for rows.Next() {
			err = rows.Scan(&request.DebitnoteNumber)
		}
	}*/
	//if err == nil {
	//query := `select mrinno, pono from dbo.inv_gc_po_mrin_master_newpg where mrinid = $1`
	...
	if err == nil {
		for rows.Next() {
			err = rows.Scan(&request.DebitnoteNumber, &request.MrinNumber, &request.PoNumber, &request.PoNumber)
		}
	}
	//}
	return err
